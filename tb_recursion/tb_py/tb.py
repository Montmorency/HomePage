...
#these are the parameters we need to fit.
    self.integrals = ['E_ss_000','E_ss_110','E_xx_000', 'E_xx_110', 'E_xx_011','E_ss_hhh',
                      'E_sx_hhh','E_sx_011','E_sx_110','E_xx_hhh','E_xy_hhh','E_xy_110', 'E_xy_011']
  ...
  def matrix_elements(self):
    ...
    xx_11 = (self.integrals['E_xx_000'] + 4.0*self.integrals['E_xx_110']*(np.cos(self.xi)*np.cos(self.eta) 
                                        + np.cos(self.xi)*np.cos(self.zeta))
    ...
    sx_12 = 4.0*self.integrals['E_sx_hhh']*(i*(np.sin(0.5*self.xi)*np.cos(0.5*self.eta)*np.cos(0.5*self.zeta))
                                              -np.cos(0.5*self.xi)*np.sin(0.5*self.eta)*np.sin(0.5*self.zeta))
    sx_21 = -1.0*np.conjg(sx_12)

    sx_11 = -4.0*self.integrals['E_sx_011']*np.sin(self.xi)*np.sin(self.eta) 
                                            + (4.0*i*self.integrals['E_sx_110']*(np.sin(self.xi)*np.cos(self.eta)
    ...
    xy_11 = (-4.0*self.integrals['E_xy_110']*np.sin(self.xi)*np.sin(self.eta) 
             -(4.0*i*self.integrals['E_xy_011']*(np.sin(self.xi)*np.cos(self.zeta)
...
#take target data as a set of k points and eigenvalues.
  target_data = []
  DiamondElements()
  d_el_k =  diamond_elements(target_data)
  d_el_k = descreen(d_el_k)
#Fit Routines
