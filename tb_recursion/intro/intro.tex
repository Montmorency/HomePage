...
sets of three decades before the 1980s. For a comprehensive introduction to 
the history of computing see George Dyson's Turing's Cathedral~\cite{dyson12} 
and the references therein. For a review of the state of computing in
1980 see Ref.~\cite{metropolis80}.

In 1936 Alan Turing published, ``On Computable Numbers, 
With An Application to the Entsheidungsproblem"
\cite{turing36a}. Turing's concerns were in the domain of formal logical but
his method of proof invoked a black box capable of advancing and rewinding a long,
segmented piece of tape and marking or clearing each segment of the tape.
This abstract black box was a computer. Turing made no formal engineering
specifications for how that box should be built. Its conception was enough.

The journey from this magic theory box to a machine capable of manipulating
numbers is the history of modern computing. All the means to store a number
reprented by binary digits and the circuitry required to add, subtract,
multiply divide, take square roots, cubed roots, and to take logs had
to be conceived, adapted, and applied. By 1953 enough progress had been made
to realize the first recognizable electronic computers.

A figure to set the scale for the rise in digitial computing is given in a report
by the U.S. Office of Naval Research: in March of 1953 there were 53 kilobytes of 
random access memory on this planet \cite{usonr53}. 
\footnote{This planet refers to Earth. Though what a happy thought
if this is being read on another planet!}.
These kilobytes were scattered across a few private machines.
They ran on punch cards and tape.

As for the types of computations that could be performed the early days of the MANIAC 
(Mathematical and Numerical Integrator and Computer) in Princeton, the circuits
were running calculations for thermonuclear applications through the day
with Nils Baricelli squeezing in the first computational biology simulations 
performed on a digital computer in the evening \cite{barricelli54, barricelli62}.

To illustrate the development from the point we may contrast the machines available at the time
he was writing to those that are widely available at the present time of writing (2018). 
