...
%
\begin{equation}
\Rs(\alpha\beta\gamma)H =  D^{\dagger}(h)HD(h)
\end{equation}
%
where $D(h)$ is a matrix representation of the symmetry element 
...
is the symmetry of the underlying Hamiltonian is preserved by construction
in the coefficient blocks $A_{n}$ and $B_{n}$\cite{inoue87}. A discussion of the
...
\begin{equation}
\mathbf{G}(E) = \bra U_{0}S|[SE-H]^{-1}|SU_{0}\ket
\end{equation}

The starting vector $u_{0}$ which in the scalar recursion
would have been a starting orbital or a linear combination of 
starting orbitals, is now a matrix of starting orbitals. 
...
$D$ matrix, or in the tensor notation, the mixed Hamiltonian (see sec.~\ref{sec:tensornotation}:
%
\begin{equation}
...
and the next block of vectors in the chain is formed by application of the Hamiltonian
and removed the previous components of the chain:
%
\begin{equation}
\label{eq:blockrec}
...
equation and the interactions present in the Hamiltonian.
These excursions begin from some prepared localized starting state
and as the electron (or phonon) hops from atom to atom
the eigenstates of the system come into sharper and sharper relief.

The closest antecendent of the recursion method was the method of moments.
...
Before concluding this chapter we present a small intuitive discussion of
the connection between moments, the recursion method and path integrals.
...
both these applications will be further elaborated
on in Chap.~\ref{chap:manybodyrec}.
