...
\end{align}
%
The forward substitution can proceed column wise or row wise. See Ref.~\cite{wilkinson68} 
...
on the passage of electrons through disordered media 
in Refs.~\cite{godin86, godin88}. 

The notation we supply here is a synthesis of the descriptions 
given in Refs.~\cite{paxton88, nex89, godin91} and is in 
...
in their formulation of the block recursion method. It is useful to incorporate 
their discussion of the underlying symmetries of the Green's function and Greenians.



\subsubsection{The General Block Recursion Procedure}
...
$\mathbf{D}$ matrix, or in the tensor notation discussed in 
Sec.~\cite{sec:tensornotation}, the mixed Hamiltonian:
%
\begin{equation}
...
and the next block in the chain is formed:
%
\begin{equation}
...
$B_{n}$ and $A_{n}$ are $p\times p$ matrices with $p$
the number of columns in the initial block matrix, or
...
%
\begin{equation}
u_{i} = \frac{1}{k_{i}}[v_{i} - \sum_{j<i}(\bra v_{i}|Su_{j}\ket)u_{j}]
\end{equation}

This can be rearranged to obtain the elements of the $B_{n}$ matrix as
