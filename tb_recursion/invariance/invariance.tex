...
The simplest chain model that can be handled analytically is the constant chain.
%
\begin{equation}
\label{eq:constantchain}
G_{0}(E) = \cfrac{1}{E-a_{0}-\cfrac{b_{0}^2}{E-a_{1}-\cfrac{b_{1}^{2}}{E-a_{2}-\cfrac{b_{2}^{2}}{...}}}},
\end{equation}
%
...
The argument of the Green's function is $E + i\delta$ for positive E the negative root is choosen
for negative energies the positive root is chosen. This gives the density of states and the real
part of the Green's function plotted in Fig.~\ref{fig:greensfuncconstchain}.

Some interesting physics can be obtained directly from the particular with reference 
to the inclusion of an impurity or vacancy in the bulk, or an atom added to the surface. To couple 
an impurity to a constant chain we allow $a_{0}$ and $b_{0}$ to vary, and the rest of the chain remains
constant. This leads to the expression:
%
\begin{equation}
G(E) = \frac{1/b_{0}}{\frac{E-a_{0}}{b_{0}} - G_{0}(E)}
\end{equation}
%
There are a few different cases depending on the relative magnitude of $a_0$ and $b_0$.
...
\caption{(a) 75 atom cuboid cluster of FCC Fe. Green atoms are in an FCC environment, and have a complete
...
\caption{Top panel: the local density of states on the central atom for the cluster in Fig.~\ref{fig:cuboid}
...
rather than via fits to band structures. 
