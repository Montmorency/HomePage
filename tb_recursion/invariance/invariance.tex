...
The off diagonal elements of the Green's function contain a lot of useful information as well.

Of particular interest are response functions \cite{terakura78} and Bond Orders \cite{tersoff86, pettifor89} 
are built out of the off diagonal elements.
...
\end{equation}
%
for a self-adjoint H, and real $\phi_{A}$ and $\phi_{B}$ the off-diagonal element of the resolvent
gives:
%
\begin{equation}
R_{AB}(E) = \frac{1}{2}[R_{++}(E)-R_{--}(E)]
\end{equation}

Pettifor defined the bond order as the integrated energy difference between a symmetric
and anti-symmetric combination of nearest neighbour orbitals \cite{pettifor89}.
%
\begin{equation}
...
\begin{equation}
G^{\pm}_{00}(E) = \bra u_{0}^{\pm}|\left[E-H\right]^{-1}|u_{0}^{\pm}\ket
\end{equation}
%
$|u^{\pm}_{0}\ket = (\frac{1}{\sqrt{2}})|\phi_{i} \pm \phi_{j}\ket$.

The Green functions can be evaluated using the recursion techniques.

The susceptibility is defined:
%
\begin{equation}
\chi_{0m,n0}(E_{F})=\frac{1}{\pi}{\rm Im}\int^{E_{F}}G^{0}_{0m}(E)G_{n0}^{0}(E)dE
\end{equation}
%

See also Finnis \cite{finnis84} for a discussion of susceptibilties.
