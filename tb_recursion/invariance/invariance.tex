...
The following treatment has been developed by Heine with reference to in particular
the work of Inglesfield in Refs.~\cite{inglesfield71,inglesfield81}
and other workers in Refs.~\cite{garcia69, velicky71, garcia71}. 
The theory of matching Green's functions impacts on pseudopotential theory.
It is also important for work on QM/MM methods where clusters to be treated
quantum mechanically are embedded in a medium where continuum mechanics are
appropriate.
...
celebrated theorem states that when a translation operator acts 
on an eigenvector of a crystal the eigenvalue is $e^{i\k\cdot\R}$.
...
metal range can be found in Refs. \cite{nieminen76}, \cite{pettifor77} and \cite{jepsen75}.
Each study is a good example of studying homologous trends. 
...
\footnote{C. Lanczos J. Res. Natl. Bur. Stand. 45, 255 (1950) 
...
in a coordinate space representation reflect the long range regularity of the material. See
Chapter \ref{chap:odlro} for further discussion of this point.
...
serves as the paradigm of all the recursion calculations that will be undertaken. The
problem is always to determine the coefficients in the expansion of the chain.
%
\begin{equation}
...
to bound and antibound states. \label{fig:impurity}}
\end{center}
\end{figure}
%
...
\begin{equation}
det|G2| = G_{21}G_{22}G_{23}G_{24}G_{25}
\end{equation}

\begin{equation}
det|G1| = G_{11}G_{12}G_{13}G_{14}G_{15}
\end{equation}

\begin{equation}
G_{i\alpha}=<i,\alpha| [EI-\bar{H}_{i\alpha}] |i, \alpha>
\end{equation}
%
with $i=1,2$ $\alpha=1,5$. The matrix  $\bar{H}_{i\alpha}$ is formed by taking 
$[EI-H_{i}]$, deleting the first $(\alpha -1)$ rows and columns to form $[EI-\barH_{i\alpha}]$,
the desired matrix element is now the in the upperleft-most corner of the matrix. 
...
The tightbinding hamilton only considers two center contributions, with the parameters coming from Pettifor
\cite{pettifor69},
...
The papers and PHD thesis of Meek describes in detail the application of the recursion method to the
computation of vibrational spectra \cite{meek76, meek78}. Electron-phonon coupling has also been
considered in this context\cite{terakura78}. The method is particularly useful for
computing vibrational spectra in disordered media where a statistical averaging procedure needs to be
...
%
\begin{equation}
m_{k} \frac{\partial^{2} u_\alpha (l\kappa)}{\partial t^{2}} = - \sum_{l'\kappa'\alpha'} \Phi_{\alpha\alpha'}(l\kappa,l'\kappa')u_{\alpha'}(l'\kappa')
\end{equation}
%
The above requires some function $V$ representing the potential energy for the given 
ionic coordinates. l, $\kappa$, and $\alpha$, represent unit cell, basis atom and 
...
\end{equation}
%
where $U_{\alpha}(\kappa;\k)$ is the amplitude of the displacement.
is considered we arrive at a system of equations:
%
\begin{equation}
...
\end{equation}
%
The vibrational eigenvalues of a cluster or crystal can then be determined from the secular equation:
%
\begin{equation}
...
The resolvent for the vibrational problem becomes the project of the
interaction matrix on to the displacement of atom $i$ in direction $\alpha$.
%
\begin{equation}
G_{i\alpha,i\alpha}(\omega^{2})= \bra i\alpha| \left[D- \omega^{2}I\right]^{-1} |i\alpha \ket
\end{equation}
%
For instance for 2 atoms in a unit cell of a perfect crystal Eq.~\ref{eq:phonon} would only
need to be computed for 6x6 matrix at every $\k$ point in the Brillouin zone to determine
the vibration spectrum. In this case there would be little need for employing a recursion method.
Well developed techniques exist to obtain the force constant matrix at every $\k$ point 
from first principles \cite{baroni01}, however this again requires an assumption about
microscopic periodicity which may not be valid. For any amount of disorder at the atomic level,
for instance in the CRN systems studied by Meeks secular determinants of $3N\times3N$ with
$N=200-300$ may need to be solved. In these situations recursion type calculation become necessary.
%
\begin{figure}
\begin{center}
...
site. The six atom system has the expected $3N-6=12$ vibration modes thus acting as a check on the numerics.}
\end{center}
\end{figure}
%
The phonon density of states and the integrated phonon density of states is depicted in Fig.~\ref{fig:phonondos} 
for a six atom molecule consisting of two atomic types. The number of sets of continued fraction coefficients
that needs to be generated is equal to $3*N_{s}$ where $N_{s}$ is the number of atoms that the phonon LDOS
or ensity of vibrational states (DOVS), is computed for and the factor of 3 is for the 
displacement in each atomic direction. The expression for the average phonon density of states
over the different matrix elements of the resolvent is first over the cartesian displacements
$\alpha$:
%
\begin{equation}
...
In Fig.~\ref{fig:phonondos} two sites are used to generate the DOVS, to a depth of $7$ 
in the continued fraction coefficients. The postprocessing of the continued 
fraction coefficients is the same as in the case of the surface and bulk DOS calculations. 
In Meeks work sufficient feature resolution in the DOVS is achieved with a depth of 20 
in the recursion steps for a cluster of 344 diamond atoms. The force constants entering 
these models were not ab initio instead being derived from a model due to Born \cite{born14}. It is now possible to obtain highly accurate force 
constants from first principles. The higher quality force constant matrices that
can be generated as a result could well be reflected in the quality (with respect to experiment)
of the features observed in the density of states.

Direct examination of the
routines involved in the computation of the Phonon DOS itself give a good feeling
for how the recursion method works. In Table \ref{tab:phonon} the various routines
and the task they accomplish are listed. These routines specify the entire problem
and with minor adaptations, (the crystal structure, force constants etc.), can 
be used to compute the phonon spectra and applied by the routines 

%Routine  & Task \\
%\texttt{EXPHNDS} & Generates recursion coefficients of phonon local density of states for cluster of atoms.\\
%\texttt{SCAN}    & 
%\texttt{DMGEN}   & Compute and accumulate the dynamical matrix between atoms . 
%\texttt{MLT}     &
...
well demonstrated by the work on Chevrel Compounds of Molybdenum Chalcogenides, \cite{bullet77},
determining the structures of the compounds in the Lave phases in Ref.~\cite{johannes76},
and explaining trends in the heats of adsorption of O and N atoms 
on the surfaces of transition metals across the row of the transition
series. 
...
provides a framework for a computationally tractable toolkit for performing highly 
