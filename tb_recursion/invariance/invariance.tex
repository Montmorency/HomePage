...
The off diagonal elements of the Green's function contain a lot of 
useful information. 
%The significance of the off-diagonal terms is that
%they encode all the information about why materials form solids in the first place
%Ref.\cite{yang62}.

Of particular interest are response functions \cite{terakura78} and 
Bond Orders \cite{tersoff86, pettifor89} are built out of the 
off diagonal elements.
%bond orders in superconductors mean non-zero elements of the local states.
\begin{equation}
\phi_{+} = \frac{1}{\sqrt{2}}(\phi_{A} + \phi_{B})
\phi_{-} = \frac{1}{\sqrt{2}}(\phi_{A} - \phi_{B})
\end{equation}
%
for a self-adjoint H, and real $\phi_{A}$ and $\phi_{B}$ 
the off-diagonal element of the resolvent gives:
%
\begin{equation}
R_{AB}(E) = \frac{1}{2}[R_{++}(E)-R_{--}(E)]
\end{equation}

Pettifor defined the bond order as the integrated energy difference 
between a symmetric and anti-symmetric combination of 
nearest neighbour orbitals \cite{pettifor89}.
%
\begin{equation}
...
chain of coefficients can be generated. However rounding errors in the evaluation
of the matrix vector products mean that numerical noise can disrupt underlying
symmetries. This can be demonstrated where there are deviations in the density
of states computed for a given orbital if the z-axis is changed arbitrarily 
which should not be the case. To alleviate this the Block recursion method.
In this method a set of initial orbitals are chosen and the the recursion
coefficients become matrices. The technique and its implementations are described 
in Refs.~\cite{jones84, inoue87, nex89, godin91, ozaki00}. To see
the method in action Paxton's study of the
phase stability in silicon Refs.~\cite{paxton87, paxton88},
and calculations of quantum transmittance through disorder media 
in Refs.~\cite{godin86, godin88}.

The difference between the scalar recursion and the block
recursion method is similar to the difference between
playing a number of notes at the same time like a 
musical chord rather than picking out single 
notes at a time as in a melody.
