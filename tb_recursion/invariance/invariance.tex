...
We can {\it represent} an arbitrary vector $|u\ket$ as:
%
\begin{equation}
...
(indexed by $i$) of the strict left and right triangular parts of $S$:
%
\begin{equation}
\label{eq:gaussseidelS}
u_{n+1}^{i} = \psi_{n+1} - Lu_{n+1}^{i} - Ru^{i-1}_{n+1}.
\end{equation}
%
Eq.~\ref{eq:gaussseidelS} involves tracking the convergence with respect
