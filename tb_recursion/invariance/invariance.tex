...
reaction of the material is recorded.
Based on the observed reactions
...
macroscopic reactions to perturbations given a model for microscopic configuration.

In any case we would like a method for calculating the response of our material system
to a probe and perturbation theory is ideally suited for this. In the language of quantum
mechanics we would like to calculate the expectation values associated with the operator 
describing a probe. The algebra of recursive perturbation theory has been worked out
in Ref.\cite{haydock77} and, of course, Vol.~35\ref{vol35} (pg.~281). Some applications
of perturbation theory to calculating operator expectations values was made in Ref.~\cite{mclean77,foulkes86}.
Haydock applied the perturbation theory of chain models to study the electronic properties of disordered materials and 
...
%
\begin{equation}
G(E) = \left[E - \hat{H} - \lambda \hat{V}\right]^{-1},
\end{equation}
%
where $\hat{V}$ is a local representation of the perturbation multiplied by some scalar $\lambda$
that determines the strength of the perturbation. 
If $\hat{V}$ breaks an underlying symmetry of the Hamiltonian the unperturbed chain sequence will
terminate.

We saw how to compute off-diagonal elements of the Green's function in Sec.~\ref{sec:tbbm} and
we shall return to this in Chapter~\ref{chap:metallurgy} when we look more closely at vibrational
and elastic properties of metals. As mentioned the full derivation of the perturbation method has been given
in Vol.~35 and Ref.~\cite{haydock77} and there is little need to reproduce
the details here. What we will include is the discussion of the calculation of matrix elements in the 
...
is worth folding it in to the present discussion.
