...
If you travel in certain materials scientist circles there is a book 
...
One may well ask what other type of space is there? 
The exposition of `real space methods' may recall the `campaign for real time' in Douglas
Adams' Hitchhiker's Guide to the Galaxy series. In that work time travel had become
so prevalent that the continuity of history had become dangerously 
fractured, with people hopping back and forth in time, 
and reality was becoming intolerably improbable. As a result an organization
...
with regards to the corollary to real space; what is referred to as reciprocal space. 
Reciprocal spaces arose as natural way of studying crystalline systems. 
Let us remove any element of mystery about reciprocal space.
The periodic structure of crystals means a natural mathematical representation of
electronic wavefunctions, electronic charge densities, and the lattices themeselves can be found 
as an expansion of those quantities by sets of sinusoidal waves. Each sinusoidal wave
can be indexed by a wavenumber $k_n = 2\pi/\lambda_n$, e.g. $\sin k_{n} x$. The index $k_n$
has units of inverse length (that is units of $1/\lambda$ so that the $\sin$ argument is dimensionless), 
i.e. a reciprocal length reciprocal space. 
Discussions of the periodic structure are simplified if we make reference directly
to these wavenumbers. For three dimensional structure we can have a three dimensional
real space vector coordinate system and can similarly construct a suitable three dimensional wavevector
coordinate system; whence, reciprocal space. 

The convenience of the reciprocal space representation meant 
that from the earliest days of X-ray crystallography 
...
The usefulness of reciprocal space is not limited to crystallography.
The periodic crystalline structure means that the electrons traveling through
a crystal also experience a periodic potential. 
Felix Bl\"och's\cite{bloch29, schiff65} famous 
theorem recognized that the character of the electronic wave function
should reflect this periodicity and he made use of the reciprocal space wavevectors
to index and obtain the electronic energies 
in a crystalline system.

So, at least for perfectly periodic systems, reciprocal space is a natural representation to work in. 
However what gains are made in mathematical convenience leads to costs in physical intuition. 
...
Let us adopt the perspective of a pragmatic chemist. 
The pragmatic chemist has the periodic table which
assigns to each atom a certain number of electrons. 
The pragmatic chemist knows that the valence electrons
are the only important electrons. The pragmatic chemists 
counts valence electrons on
one atom, valence electrons on another atom, and then asks 
what happens when we place those two atoms
next to each other. For small molecules, or small regions of large molecules, 
the hope is this approach 
will lead somewhere. This hope is well founded simple arguments
based on `closed shells' and the shapes of atomic orbitals 
helps rationalize a significant
...
and the technique for \textit{calculating materials properties in real space}.
The method is applicable in situations where Bl\"och's theorem no longer applies;
a situation which arises at surfaces, in glasses, in disordered structures, 
in structures with impurities,vacancies, defects, and dislocations. 
In short the method described is applicable 
to any realistic physical situation 
and in all cases of engineering relevance.
...
be explained in the following. The focus is largely on technique at this stage
and more concrete applications will be made in later chapters.
...
properties in real space terms or what has been called 
\textit{the local atomic environment}. This justification stems from
...
Finally, the reader of this chaper will quickly recognize that the recursion method
is closely connected to the mathematical theory of continued fractions and orthogonal polynomials.
The necessary mathematical exposition has been included in Appendix~\ref{app:mathsandalgs} 
...
theorem. The essential observation which underpins the invariance theorem 
is that there is a formal analogy between electrons considered as waves in a solid 
and the black body spectrum of electromagnetic radiation in a hollow cavity.
The existence of this analogy, first recognized by Jacques Friedel\cite{friedel54},
means we can make use of an important property
of the blackbody spectrum that was first demonstrated by Hermann Weyl and Max Von Laue.

In the case of electromagnetic radiation Weyl and Von Laue
demonstrated that the local electromagnetic properties of the radiation system 
is formally insensitive to the boundary 
conditions of the system, so long as the boundary is more than a few
wavelengths. We can state this informally as `what goes on in the middle
of a reflecting box is not effected by the shape of the box as long as 
the walls of the box are far away.' As we shall see we can make these terms
quite precise.

The origins of this result go back to the early days of quantum theory.
In particular the work of Hermann Weyl, a mathematical prodigy and a 
student of Hilbert and his school 
at G\"ottingen and, independently, Max von Laue , 
whose work was contemporaneous with his 
nobel winning discovery of X-ray diffraction
by crystals. Historically these results were arrived at shortly after Planck 
work in radiation theory and Einstein's theory of special relativity had been described.

A little historical digression is merited here.
Ewald, who also made important contributions to X-ray crystallography,
has authored a biographical sketch of Von Laue\cite{ewald60}. 
As he writes, in the German physics community c.a. 1910 there seemed to be a consensus
that the thermal motion of atoms would frustrate the production of any 
...
He had the experimental work carried out by two of his assistant W. Friedrich and P. Knipping.
As it happened, \textit{they were able} to obtain clear diffraction patterns and
Von Laue was quickly able to provide a mathematical interpretation of the
diffraction spots. (No doubt the former basis for the theoretical objections 
to the effect were altered to demonstrate that the diffraction must in fact occur.) 
Von Laue's formulation in terms of a three dimensional 
diffraction grating was quickly complimented by the work of 
W. H. and W.L. Bragg, which Von Laue viewed as essentially equivalent to his own, 
of the radiation beams being scattering off of densely packed atomic planes.
Ewald articulates the impact of Von Laue and the Braggs' work beautifully:
%
\begin{quote}
Together, these two ideas (those of Von Laue and the Braggs) led to the construction of the 
...
scientists whom, whether they came from physics or chemistry, were mainly interested
in deciphering the atomic arrangement in crystals and thereby to delve deeper 
than ever before into the mysteries of chemical interaction.
\end{quote}
%
...
role it has played in diffraction phenomena 
and the elucidation of the atomic structure of materials.

Before we discuss Von Laue's derivation of what we have come to call 
the `invariance principle' we shall introduce the local density of states. 

\subsection{The Electronic Blackbody: Local Density of States}
\label{sec:matchinggreens}
%\section{Green's Functions}
In any macroscopic material system there is an enormous number of atomic degrees of freedom.
If we are no longer allowing ourselves to exploit the perfectly periodic structure
to place restrictions on the independent degrees of freedom in the system 
we will have to account for each one
individually. In order to make progress we need to find some functional 
distributions which help us account for all these degrees of freedom.
In the case of the Blackbody we have Planck's distribution which describes
the excitation of electromagentic waves as a function
of temperature. 
For reference the local density of modes for black-body radiation can be written\cite{annett94}:
%
\begin{equation}
\rho(\omega,\r)= \sum_{i} |\bra \r| \psi_{i}\ket|^{2}\delta(\omega-\omega_{i}),
\end{equation}
%
with $\psi_{i}$ the eigenfunction (of an electromagnetic wave) 
for an eigenvalue (frequency) $\omega_{i}$.
The analogous distribution we shall use 
...
\end{eqnarray}
%
This function accounts for the distribution of the electrons 
across a range of energies in a material system. 
For example for a hydrogen atom the density of
states will have a delta function located at each of the permissible energy levels
of the hydrogen atom. In the case of hydrogen we can compute the discrete values for $E_{n}$ 
to good accuracy fairly easily using analytic techniques. 
In a solid, as one might expect, involving many atoms arranged in complex patterns, 
the spectrum becomes more complicated and this is reflected in the distribution of
the electronic energy levels. This complexity ensures continuing work 
for materials scientist whose job it is 
to calculate and interpret these distributions.
...
an additional spatial variable $\r$ which parameterizes the spatial dependence 
...
At this stage we will introduce, somewhat abruptly, 
the Green's function. In chapter~\ref{chap:gw} we will define the Green's function
with a little more mathematical precision but for present purposes we will
be a little more casual. We will define our Green's function as an energy dependent
matrix element of the electronic wavefunctions of the system with the Hamiltonian:
%
\begin{equation}
    G(\r,\r'; E) = \bra\phi_{i\alpha}(\r)|[E + i\delta -H]^{-1}|\phi_{i\alpha}(\r')\ket.
\end{equation}
%

Friedel defined the connection between Green's 
function and local density of states as\cite{friedel54}:
%
\begin{equation}
        \label{eq:friedel}
    n_{i\alpha}(E,\r) = -\frac{1}{\pi} {\rm Im}\bra \phi_{i\alpha}(\r) |[E + i\delta -H]^{-1}|\phi_{i\alpha}(\r)\ket,
\end{equation}
%
where $\r$ and $\r'$ have been identified. 
The state $\phi$ which we project on to can be anything we wish. It can be made up of some linear combination 
of local spherically symmetric atom centered orbitals, bond orbitals, 
Wannier functions, atomic diplacements, electron spin, or any localized quantity we wish to define. 
The Green's function is thus a very useful way of encoding information about the spatial and energetic
distribution of physical quantities. We are only constrained by how easily we can evaluate the matrix elements
that arise. The work of evaluating matrix elements can become quite tedious and this is why we often resort
to the use of computers. 

Having introduced the local density of states and the Green's function 
we now return to our original task which was to 
demonstrate that the sum in Eq.~\ref{eq:ldos} evaluated at 
the point $\r$ is insensitive to changes in the system more than 
a few characteristic wavelengths away.

We can be a little more precise about  
what constitutes a characteristic wavelength.
Let us take the elastic case as Von Laue did 
in 1912; `we find $h\nu_{m}/k = 1860$ leading to a frequency of
$v_{m}=3.97 10^{13}$sec$^{-1}$,
with the speed of sound of the order of magnitude of
$10^{6}$ cm sec$^{-1}$we can estimate the associated wavelength
as being on the order of magnitude
$10^{-7}$ cm.' So we can concludes for elastic waves 
it would appear we can rely on invariance 
for systems larger than a few nanometers. 
%W. Nernst u. F. A. Lindemann, Berl. Ber. p.~1160. 1912.

By demonstrating this invariance we will have obtained a huge reduction
in the computational effort required. To accomplish 
this we must exploit the analogy with blackbody radiation,
and to do this we enlist the help of Weyl and Von Laue.
...
but we will mention a few points in connection with Weyl's work. 

Weyl, who presented his results in 1912, 
was, as we have mentioned, a student of Hilbert's and was extremely well versed in the 
ideas of Hilbert spaces and Green's functions. 
...
compared with Von Laue's classical demonstration. He organizes his arguments in
terms of sets of eigenvalues of integral equations. 
There is a distinct topological character to some of his arguments and
it is interesting to note that he prepared an influential monograph on Riemann surfaces
at the same time as this work on black body radiation\cite{weyl97}. As
is the mathematicians wont his tendency seemed to be towards
a mathematically abstract picture of oscillations on arbitrary manifolds; but he grounded
his work in considerations of electromagnetics and elasticity and examined the consequence
of Dirichlet's principle on wave equations.
His paper is titled the,`Asymptotic distribution law 
of the eigenvalues of linear partial differential equations with an application
to the \textit{Holraumstrahlung}'. The germanic compound noun \textit{holraumstrahlung}
translating as `radiation in a cavity with a small hole in it'. 
This is often, although more obliquely, called a `blackbody' in English. 
...
At this point we only wish to stress that Weyl's asymptotic relations 
\textit{ are not restricted to electromagnetic wave
phenomenon. In fact, the asymptotic properties are valid in any
situation where we have a linear wave equation leading to vibrations 
on an oscillating membrane:
electromagnetic, electronic, or elastic. }
As we shall see many of the techniques described in these notes can
be applied without any significant modification anywhere we have a linearized
wave equation. Haydock has been one of the prime expositors of this approach 
exploiting the invariance property of wave equations to study electronic,
electromagnetic, and elasto-mechanical phenomena. 

Before moving to Von Laue's derivation we shall discuss that of 
Volker Heine.
His derivation summarized the lines of development 
of Garcia-Moliner and Rubio, and, Velicky and Bartos, Refs.~\cite{garcia69, velicky71, garcia71},
along with the work of Inglesfield in Refs.~\cite{inglesfield71,inglesfield81}.
The end result of his analysis is he obtains an expression for the Green's function that can be
...
Let us take Fig.~\ref{fig:pseudo} as an example. 
We have partitioned our system into regions $G_{A}$ 
near the atoms and another region $G_{B}$ that is valid in the interstitial
regions between atoms. 
In the interstitial regions we wish to describe the electronic states in terms of plane waves.
At the boundary between the two regions we must match the Green's function for the atomic system $G_{A}$, 
(which we have worked out by some other means analytic or otherwise),
at a particular radial cutoff and across a specific energy window. 
%
\begin{figure}
\begin{center}
...
and $G_{B}$ explicitly at each stage of the calculation until self-consistency is obtained.\label{fig:pseudo}}
\end{center}
\end{figure}
%
If we can construct a suitable way of pasting together the regions $G_A$ and $G_B$
we will have obtained a representation of the local density of states, via Eq.~\ref{eq:friedel},
and a host of other interesting properties that follows from this knowledge. The requirement is that the 
we can effective partition the Green's functions that is that the "boundary corrections" remain 
consistently small.

In fact, an intuitive grasp of the theory of matching Green's functions is a key component of a material
...
the interaction between valence electrons and core electrons, dislocations, grain boundaries, all
involve in one way or another matching Green's functions across a surface.

While Heine's derivation is internally consistent we 
do find it useful to pick out two statements he has made which suggest
a useful complement. In Vol.~35 Heine points out that his derivation of the invariance principle,
`is completely analogous to the theory of Fresnel half-period zones or the Cornu spiral,'
however that, `he knows of no general discussion in the literature'. 
As it turns out Von Laue's original work as it applies to electromagnetic radiation
is perfectly suited as a general discussion. It also places a slightly different emphasis
on certain aspects of the derivation and it is well worth revisiting some of the points
developed in his original work. 

We now turn to Von Laue. Von Laue's derivation of the invariance principle originates 
to a much greater degree in experimental and physical
intuition. 
Von Laue begins his derivation by making an analogy with the optical resolving power of a lens.
The reader may notice that we are starting to accumulate analogies:
the first between the electronic wavefunctions and electromagnetic radiation, and now 
between the study of electromagnetic waves and the resolutions of lenses, not to mention
the connection between electromagnetic waves and elastic waves which was recognized by Weyl and Von Laue
as well. Arguably Weyl's work comes the closest 
to examining the underlying mathematical structure which connects all 
these phenomena.

To proceed with the optical analogy Von Laue set up the spherical coordinate system pictured
in Fig.~\ref{fig:vonlaue}. Out of interest his discussion follows closely
analysis modeled on Debye's study of Cornu spirals
see Fig.~\ref{fig:debye} for the original Fig. from Debye's work\cite{debye1908}.
%
\begin{figure}
\includegraphics{./invariance/VonLaueDiagram.png}
\caption{Von Laue's geometric construction from Ref.~\cite{vonlaue14}.\label{fig:vonlaue}}
\end{figure}
%
\begin{figure}
\includegraphics[width=\columnwidth]{./invariance/DebyeCoordinates.png}
\caption{Debye's geometric construction from Ref.~\cite{debye1908}.\label{fig:debye}}
\end{figure}

It is interesting that Von Laue's work predates the wave theory of electrons yet
the fundamental character
of these considerations to wave phenomenon in general 
means that his analysis can be carried over directly with very little modification.

We have considered it useful to include a more lengthy discussion of Von Laue's 
derivation. This is partly to be more precise about what the invariance principle
actually states. Frequently Weyl and Von Laue's papers are cited with the claim
`they demonstrate an exponential insensitivity of the local behaviour of 
wave equations to the boundary conditions'. In fact, in neither Von Laue nor Weyl's papers,
does an explicit decaying exponential appear. The truth is more subtle. 
What is demonstrated is that by carefully setting up equations 
that described the behaviour of the wave 
phenomenon in a small region of space for a small spectral width, they
are able to show that for large cavity sizes the statistical properties of the 
cavity radiation are insensitive to the boundary conditions due to destructive intereference effects. 
The formulations resemble more closely what have come to be called
windowed Fourier transforms and wavelets; mathematical constructions which 
obtain an optimized description of a wave pattern in terms of 
the decay of the signal in both 
real and reciprocal space.

Whether one favours Weyl's approach or Von Laue's or Heine's specialization of these results
\textit{the essential point is that 
while the normal modes of a system can be quite sensitive to disturbances, the 
local density of modes, averaged over a small frequency interval, 
is a stable quantity that is formally insensitive to disturbances more than a 
few wavelengths away.} It is this point that will allow us to study the 
properties of the electronic and elasto-mechanical waves in materials
in terms of the local atomic structure.

\section{Bl\"och's Theorem and Slater-Koster Tight Binding}
...
Bl\"och's theorem encapsulates this idea by required the electronic
eigenstates of the crystal be of the form:
%
\[
  \psi_{k} = e^{ik\r}u_n{\r},  \quad {\rm where}, \quad u_{n}(\r) = u_{n}(\r+\R),
\]
%
where $\R$ is a crystal lattice vector (i.e. a periodic translation). 
When a translation operator $\hat{T}$ acts 
on a Bl\"och state, sending $\r\rightarrow \r+\R$, 
then the wavefunction is multiplied by a scalar
value $e^{i\k\cdot\R}$. This is precisely the condition for a vector to be an
eigenvector.
...
The matrix elements between Bloch sums can be written: 
%
\begin{equation}
        \sum_{\R_{j}} e^{i\k\cdot(\R_{j}-\R_{i})}\times\int \phi^{*}_{n}(\r-\R_{i})\hat{H}\phi_{m}(\r-\R_{j})dV
\end{equation}
%
\footnote{Slater and Koster anticipated that the integrals which arose and the Bl\"och 
sums themselves could be performed with the help of computers: ``The possibility is not excluded 
...
The work of Slater-Koster demonstrated how localized atomic
states built out of the $s$,$p$, and $d$ waves familiar from atomic physics, could, 
when modulated by a wave of the form $e^{i\k\cdot\R}$ interfere 
...
due to this modulation can give rise to band structures. 

The computational usefulness of approach comes in the two-center approximation. That is that the
matrix elements between orbitals on neighbouring atoms can be written as sums of invariant scalars,
which correspond to the radial integrals of the matrix elements, multiplied by numerical prefactors 
determined by the relative angular position of two atoms. 
Slater and Koster 
tabulated the angular prefactors and then worked out the numerical value of the radial integrals
via an interpolation method.
Experimental data on the energy levels of electrons in crystals resolved by the 
wavevector $\k$ were available and Slater and Kosters worked through the diamond, FCC, and BCC
structures to produce band structures from their interpolation scheme. The mathematical
details are discussed further in Appendix~\cite{app:appen2}.

Although it is often presented to undergraduates 
as an early approximation scheme of historical interest 
the Slater-Koster picture of electronic
structure deserves to be regarded as a major step forward 
in our ability to understand the properties of materials.

\subsection{Computational Implementation}
Let us take a concrete example proposed by Kelly\cite{vol35}. 
This should make the operation of the tight binding scheme a litter clearer:
%
\begin{quote}
As input data we require a convenient local representation of $H$. In simple crystals,
this is most easily given in terms of interactions within a unit cell and between neighboring cells.
...
The operation by which $H|u_{n}\ket$ is computed can be written explicitly as:
%
\begin{equation}
\label{eq:fullhamhop}
...
The essence of the recursion method is that the 
summation in Eq.~\ref{eq:fullhamhop} does not run over every matrix element of the Hamiltonian.
...
about the pre-dominance of the local atomic environment. 

Computationally, and I suppose philosophically, Eq.~\ref{eq:hop} is at the
...
vectors are handled. The idea is that if our operator can be represented by a discrete number of small 
$m\times m$ matrices we can efficiently obtain numerical data describing the material properties
\textit{and} an understanding of the underlying physics in terms of the local atomic environment.

\section{Particles and Waves: Transition Metal Tight Binding}
\label{sec:tmtb}
In this section we would like to build up a little physical intuition for what
the Slater-Koster integrals actually represent, that is a sense of what the physical 
picture is describing and how different electronic states might `look' in a solid.

We begin with the uncertainty principle. This allows for the estimation of the rest
...
For $d$ electrons we can begin by consideration of Fig.~\ref{fig:datoms}.
(Bullet discusses some of the problems with treating d electrons in a tight binding 
formalism (Sec. IV 13 Solid State Physics Vol. 35).)
%
\begin{figure}
...
tightly bound d-electrons and relatively diffuse s-p orbitals hybridizing 
...
significant. 
%This isn't entirely surprising. We are considering a periodic crystalline solid
...
The `best' approximations to the explicit 
formulas for $dd\sigma$, $dd\pi$ and $dd\delta$ and the phase
shift $d_{0}'$ of the diagonal tight-binding elements 
These difficulty have been addressed in a number of works 
\cite{hubbard67,mueller67}, \cite{hubbard68}, \cite{hubbard69} 
\footnote{Another computational
...
main storage capacity of those machines was 524 kilobytes.} 
The problem being to define an
energy independent Hamiltonian, amenable to tight binding for the d electrons.
%See Fig.~\ref{fig:ibm360}%for a (grainy) snapshot of what that computer looked like.},
%and J. Hubbard, ibid 2, 1222 (1969). 
...
Pettifor's work produced a justifiable scheme for computing the 
two center tight binding integrals of Slater
and Koster, $dd\sigma$, $dd\pi$, $dd\delta$ in terms of the position and 
halfwidth of the resonance:
$\epsilon_0$ and $\Gamma$. 

The achievement of these collected papers is quite remarkable.
Using primarily analytic techniques the workers determined 
the positions of atomic eigenvalues, band widths, and the effect 
of atomic scattering potentials for transition metals. In 
...
of numerical values via graphical methods. 
%All in all 
%a laborious but fruitful task resulting
%in energy independent matrix elements localized to 
...
development of the recursion method in the 1970s. By the 1980s 
generalized tables for tight binding parameters 
across the transition metal series had been produced in Ref.~\cite{harrison80}. We have 
collected some numerical values for tight binding parameters 
in Appendix~\ref{appen:grouptheory}.

\subsection{Mathematical Origin of the Recursion Method}
The trick of the recursion method, if you like to think of things in terms of tricks, 
or its generating feature, if you prefer, is that from the outset 
we restrict our inquiry to a single `Greenian', i.e., i.e. a single matrix element
...
Now let us recall that if we are given a tridiagonal matrix of the form:
%
\begin{equation}
H = \begin{bmatrix}
a_0 & b_1 & 0 & 0 & 0 &  ...\\
...
\footnote{\index{C. Lanczos}\cite{lanczos50}
used the chain construction as a way of diagonalizing matrices
...
What is then is required is a way for us to generate a tri-diagonal matrix
in the form of Eq.~\ref{eq:} . Just such a method is given to us
...
where we can recognize a diagonal `on-site' contribution:
%
\begin{equation}
a_{n} = \bra u_{n}|H|u_{n} \ket,
\end{equation}
%
a projection on to the previous state in the chain: 
%
\begin{equation}
b_{n} = \bra u_{n}|H|u_{n-1} \ket,
\end{equation}
%
and a projection on to the next state of the chain $b_{n+1}u_{n+1}$.

This derivation is similar to that given in Vol.~35. In the appendix we pursue an
alternative derivation of the recursion method that makes the connection
with some different mathematical techniques a little more explicit. 
However the present derivation is good enough to satisfy our mathematical curiousity.

\section{Analytic Recursion}
To get a flavour for what we can calculate with the recursion method we will look
at some simple physical situations which permit an analytic solution. These 
are the cases where we have a simple geometry and a simple expression for the recursion
coefficients. Although they are mainly pedagogical there is a lot 
of physical content that can be gleaned from these simple models. 

The first and simplest chain model that can be handled analytically is the constant chain.
This serves as the paradigm of all the recursion calculations that will be undertaken. 
It demonstrates how we can obtain energy bands from discrete hops. Subsequent models
show the interesting effects that arise when an impurity interacts with a band of 
electronic states and the types of resonances that may arise. 
...
As we mentioned a great deal of interesting physics can be obtained directly from the chain model. 
For instance, to couple an impurity to a constant chain we allow the first two coefficients $a_{0}$ and $b_{1}$ to vary, 
and the rest of the chain to remain constant. This leads to the expression:
%
\begin{equation}
...
\section{Asymptotics, Termination, and Integration}
...
Integrals over the density of states 

See the routines \texttt{DENCRS} and \texttt{DENSQ} 
...
\section{\texttt{ORPEEL}: Orbital Peeling}
\label{sec:orpeel}
...
\section{Off-Diagonal Elements of the Green's Function}
\label{sec:offdiagelements}
...
\section{Non-Orthogonal Recursion}
...
\section{\texttt{RECNO}: Recursion Non-Orthogonal Algorithm}
\label{sec:recno}
...
\section{\texttt{BLKRECLIB}: Block Recursion}
\label{sec:blkreclib}
...
\subsection{Matrix Continued Fractions}
...
\section{Questions}
\begin{itemize}
\item From Local to Bl\"oched. Use Bl\"och's theorem to obtain the density of states
        for the constant chain. Let the atoms be spaced with a distance $L$
        and the hopping energy by b. Bl\"och's theorem requires that the phase
        increases with each step along the chain \psi_{k}(r) = e^{-i(n+1)x/L}. 
        The permissible energies come from solving the secular equation for a  
        $2\times2$ hamiltonian with off diagonal elements b.
