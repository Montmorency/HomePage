...
If you travel in certain material's scientists circles there is a book 
that comes up for discussion now and then and is  
generally referred to only as `Vol.~35'. 
The full title of this remarkable book 
is \textit{Solid State Physics: Advances in Research and Applications
Volume 35}. It appeared in 1980 and is
dedicated to, `the exposition of real space methods.' The editors 
of the series were Henry Ehrenreich, whom we met in the previous chapter,
Frederick Seitz and David Turnbull.

What is meant by, `...the exposition of real space methods.'?
To some skeptics the term `real space' (what other type of space is there?) 
recalls the `campaign for real time' in Douglas
Adam's Hitchhikers Guide to the Galaxy series. In that work time travel had become
so prevalent that the continuity of existence was becoming dangerously 
fractured and reality was becoming intolerably improbably. 
For Materials scientist real space is the corollary of reciprocal space.
The use of reciprocal spaces arises naturally in crystalline systems, 
which have some periodic structure. The interpretation
of diffraction patterns is most easily accomplished in reciprocal space. That
is we look for the characteristic diffraction spots which give indications of the 
spacing of the atomic planes. Electronic waves in crystals experience a similarly
periodic potential and Bl\"ochs theorem allows us to enumerate and solve for the 
eigenvalues of the electronic system in reciprocal space as well. For perfectly
periodic systems reciprocal space is a natural representation to work in. However
what gains are made in mathematical convenience leads to costs in physical intuition. 

In real space we adopt the perspective of the chemist. The chemist places electrons on
one atom electrons on another atom and asks what happens when we place those two atoms
next to each other? For small molecules we may hope this approach will lead us somewhere
but as the size of our systems increases we must account for the diffusion of the electrons
throughout the system to obtain an accurate quantitative and qualitative picture of the 
physics. What we require is an intermediate representation between a strictly localized
picture of bonding electrons attached to individual atoms and Bloch's picture of 
electron waves delocalized throughout an entire crystal.

One of the primary early expositors of the
...
`owing to an overdose of X-ray scattering.' \cite{sutton}.

Vol.~35 is dedicated to electronic structure
...
The real space method is applicable in situations where Bl\"och's theorem no longer holds.
This is a situation which arises at surfaces, in glasses, disordered structures, 
in structures with impurities
vacancies, defects and dislocations. In short the method is applicable
...
We have a few objectives in this chapter. The first is to 
attempt to summarize the incredible range and
detail of theory and application covered in Vol.~35 of Solid State Physics.
We have also collected work directly related to the recursion method 
which appeared shortly after Vol.~35. In this way we can collect 
all the developments in the technique: local density of states calculations,
off-diagonal elements, block recursion, perturbation theory, and non-orthogonal recursion,
under one roof.
The second purpose is to indicate where the attached computer 
routines in the \texttt{CamRecLib} package can be employed. 
...
The reader can then adopt these examples to perform `industrial' calculations 
for whatever applications may interest them.

To begin we discuss the physical justification for a description of a materials 
properties in terms of the local environment. This justification stems from
what Volker Heine has called the `Invariance Principle'. We then work through
the derivation of the recursion method and the algebra underlying its application
to the physical problems we are interested in. As it turns out the recursion method
...
Weyl demonstrated demonstrated the total density
...
under consideration\cite{weyl1912}. Von Laue 
[1914 Ann. Phys. Lpz 44 1197] demonstrated that the local density of modes at 
a particular point are exponentially insensitive to the cavity walls.

The analogy has been developed by Friedel \cite{friedel54} with 
the electromagnetic waves of the blackbody spectrum
swapped for electronic wavefunctions. In this swap
the probability density for finding an electron at a given point
with a given energy replaces the local density 
of normal modes\cite{annett94}.
...
The normal modes of a system can be quite sensitive to disturbances 
but the 
local density of modes, averaged over a small frequency interval, 
is a stable quantity that is
insensitive to disturbances more than a 
