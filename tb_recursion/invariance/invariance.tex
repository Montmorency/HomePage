...
The reader may notice that at this stage a proliferation of analogy:
the first between the electron wavefunctions and electromagnetic radiation, and now 
...
as well. The reader may consult the work of Weyl who examined the 
underlying mathematical structure which connects all 
these phenomena more closely. 

To proceed with the optical analogy Von Laue set up the spherical coordinate system pictured
in Figs.~\ref{fig:vonlaue} modeled on Debye's study of Cornu spirals and the 
interference patterns of diffraced electromagnetic waves
...
properties of the electronic, elasto-mechanical waves in materials
...
%
\[
  \psi_{k} = e^{ik\r}u_n{\r}  \quad {\rm where} u_{n}(\r) = u_{n}(\r+\R),
\]
%
where $\R$ is a crystal lattice vector 
preserving the periodicity of the crystalline system.
When a translation operator $\hat{T}$ acts 
on a Bl\"och state sending $\r\rightarrow \r+\R$ 
of a crystal then the wavefunction is multiplied by a scalar
...
at arbitrary $\k$ points. In fact, Slater and Koster anticipated that the integrals 
which arose and the Bl\"och 
...
The great hope of these notes is that the better we understand how electrons move through crystals the better
able we are to understand the properties of materials. Although it is 
often presented as a crude approximation scheme or interpolationg scheme 
the Slater-Koster picture of electronic
structure has proven to be a major step forward 
in our ability to understand the properties of materials.

%Determining the coefficients involved in the 
...
It is worthwhile considering the achievement of these collected papers.
...
of numerical values via graphical methods. All in all 
...
across the transition metal series has been produced in Ref.~\cite{harrison80}. We have 
collected some numerical values for tight binding parameters 
in \ref{appen:grouptheory}.
...
to tridiagonal matrices and expansions in terms of determinants the present derivation
