...
\footnote{It is  highly recommended to read the obituary of Coulson published in 
Proc. Roy. Soc \cite{altmann74}. This work gives a comprehensive
overview how wave mechanics were imported to the study of 
complex molecules at the earliest stage. Coulson's 
...
``Last night." Coulson may have performed the first iterative self-consistent field calculations 
on a molecule, $H_2$, by hand (with the possible help of a £25 Brunsviga calculator)
in Ref.~\cite{coulson38}. Coulson passed away in 1974 as the recursion method was gaining
in ascendancy. Coulson's life work spans diverse fields touching on numerical methods 
\cite{coulson37,coulson42}, random walks as they apply to a certain type of slug traversing a surface 
\cite{coulson47}, aromatic compounds, polymers, dielectric media, and theology. 
The application of the methods reviewed in the present work to his 
fields of endeavour would constitute a textbook onto itself.}
...
been that they are orthogonal. The most intuitive basis set to choose
Unfortunately spherical harmonics centered on neighbouring atoms
will not be orthogonal functions. 

L\"owdin functions are a class of orthogonal function
as are Wannier functions but the price to be paid for
obtaining these functions is that of a first principles calculation
these matters will be discussed further in Chap.\ref{chap:wannier}.
...
and in the same paper he notes other work that had been done 
for forming localized wavefunctions which may act as molecular building 
blocks \cite{edmiston63}. Though we begin to digress it
is clear that for physical and numerical accuracy we
require a non-orthogonal basis set but for intuition
or heuristic purposes we may wish to continue thinking
...
S should hopefully have some nice properties like its diagonal
elements are unity and that it possesses small off-diagonal 
elements that fall off quickly in real space.
...
The elements of the inverse overlap matrix are chosen using
the definition of a matrix inverse as that which produces
the identity matrix upon multiplication with its own inverse.

The dual set of basis vectors is then defined:
%
\begin{equation}
...
The two most important aspects of the basis are how we can reprsent
an arbitrary vector $|u\ket$ which we will call {\it representation}
(done using either set of vectors):
%
\begin{equation}
...
The arbitrary vectors might be the eigenvectors which we wish 
to obtain in which case we need to diagonalize
the Hamiltonian and the {\it norm} should be one.
...
indexed vectors is illustrated by considering how the Schr\"odinger
...
states and allowing us to describe the diffusion of an s state or
a d-orbital localized on a particular atom.
...
%
\begin{equation}
\bra U^{T}S|[S\epsilon-M]^{-1}|SU\ket
\end{equation}
%
...
\end{align}
%
The forward substitution can proceed column wise or row wise. See Ref.~\cite{wilkinson88} 
for a discussion of the stability of this procedure.
...
Refs.~\cite{paxton88, nex89, godin91} and is in accord with the accompanying 
\texttt{CamRecLib} routines.

For some historical insight into the need for Block recursion we quote Finnis \cite{andersen89}
...
same time like a musical chord rather than picking out single notes at a time.
