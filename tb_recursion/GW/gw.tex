...
\begin{equation}
\frac{\delta}{\delta \rho} \left[E[\rho] - \mu(\int \rho(\r)d\r - N)\right],
\end{equation}
which leads to 
\begin{equation}
...
useful collection of functional definitions collected into one
place. The Hohenberg-Kohn-Sham Energy written in its pure functional form,
...
%
\begin{equation}
C_{in(\r,\r') = \frac{1}{|\r-\r'|} + \frac{\delta^{2}E_{xc}[\rho]}{\delta rho \delta rho'}
\end{equation}
...
\subsection{Harris-Foulke Generalized and the Tightbing Hamiltonian}
Write 3.48
...
want to calculate and how accurately you would like to calculate it. However the functional
is only minimized when $\rho^{in} = \rho^{out}$ and $V_{eff}=V_{in}$.
