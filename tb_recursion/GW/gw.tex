...
for a total energy functional of the electronic density.
We are also in possession of another constraint the conservation of charge:
$\int\rho(\r)d^{3}\r = N$. The integral of the charge
...
There are some additional technical details that arise at this point which we need only make reference to.
...
We now compare this directly with the HKS expression for the total energy:
%
\begin{equation}
...
We now expand the total energy functional around a stationary point:
%
\begin{equation}
...
There is a great deal of scope for connecting second order Kohn-Sham functionals,
linear response theory, and the self-consistent tight binding method.
For a proper development of these points the reader must consult Finnis.
...
To proceed Foulkes and Haydock make the assumption that if only small areas of the material have 
significant charge density contributions arising from three atom 
...
To conclude this section we add a brief note on the crucial difference between
repulsive band models and repulsive bond models. Finnis, in his textbook, makes
a distinction between $U(|\R_I - \R_J|)$ as a parametric two-center 
functional fit of the double counting terms in Eq.~\ref{eq:} which he terms the Repulsive Band Model.
He distinguishes this from the repulsive contribution that arises in the Tight Binding Bond Model\cite{sutton88}, 
favouring the latter. The distinction is made clear by noting in the Bond Model the total energy which is described
is the cohesive energy: that is the difference between the energy of the bonded system
and the same system decomposed into individual atoms. In the Band Model the repulsive contribution
is parameterizing a total system energy which will need to be defined by technical details of the calculation,
and the charge transfer effects implicit in the use of the Harris-Foulkes eigenvalues are exagerrated.
...
As will be seen we will make them explicit in the construction of our local models. 
It will be seen that our splitting of the energy functional is 
consistent with the repulsive bond model which, in agreement with Finnis, we find to be the less 
repulsive\footnote{In the non-physical sense of the word repulsive.} of the two choices.
