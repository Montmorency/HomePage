...
Before we conclude this chapter we will introduce some diagrammatic representations due
to Methfessel which may help the reader picture what we have discussed\cite{methfessel95}.
The process involved in doing a self consistent calculation can be represented:
%
\[\xymatrix{
	\cdots \ar[r] & V^{0} \ar[r] & n^{0} \ar[r] & V^{1} \ar[r] & n^{1} \ar[r]  & \cdots \\
}
\]
%
These sort of diagrams are similar to exact sequences \cite{whitehead50} in algebraic 
topology. The chain continues until it stabilizes at $n^{i+1}=n^{i}$. In practice
there might be a hundred steps in the chain before this stabilization occurs and
the expense of manipulating the representations of $V$ and $n$ can be considerable.

We can wrap the sequence Eq.~\ref{eq:exactseq1} and inject projection operators:
\[\xymatrix{
...
From this sequence we are looking to define projection operations, 
$\hat{P}_{n}$ and $\hat{P}_{V}$, that preserve as much of the 
exactness of the original sequence as possible. If we can build the density out
of a few atom centered functions and represent our potentials with a few scalars,
possibly with some simple functional dependence, we will have obtained
a very useful compression of the ab initio data.
...
system to self-consistency over and over again. The input, output density, 
