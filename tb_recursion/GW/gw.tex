...
%
\begin{equation}
U = U_{NN} + \sum_{n} E_{n} - \frac{1}{2}\int V_{N}(\r)\rho(\r)d^{3}\r
-\frac{1}{2}\int\int\sum_{n} \psi_{n}^{*}(r)V_{xc}(\r,\r';E_{n})\psi_{n}(\r')d^{3}\r d^{3}\r'
\end{equation}
%
Calculating this quantity is a formidable challenge. We have
...
\section{Harris-Foulkes and Tight Binding}
...
to go out and do alot of damage.
...
\begin{equation}
\tilde{V}(x) = V^{H}(x) + \mu^{\rm nf}_{\rm xc}(x) + V^{ion}(x)
\end{equation}
...
%
\begin{equation}
E_{R} = \sum_{n}a_{n}\tilde{\epsilon} - \int dx n_{f}(x)[\frac{1}{2}V^{H}(x) + \mu^{nf}_{xc}(x)] + E^{xc}[n_f]+ E^{\rm ion}
\end{equation}
%
...
fact that when the errors introduced by the approximation are analyzed the errors
are proportional to $\delta n(r)^2$ and not linear in the density. If a good
guess at the density can be found, good in the sense the combined 
density is almost equal to the true self-consistent 
density $n_{f}(r) \approx n(r)$, we have some grounds to think that 
the computed total energies will be as well.
...
of a self-consistent loop. The idea had been tried in a few different contexts
\cite{harris85, foulkes87, foulkes89, haydock11}.

The extremal properties of a larger function are summarised in Ref.~\cite{haydock11}.
The variation with respect to density is equal to the potential:
%
\begin{equation}
\frac{\Phi^{TB}}{\delta n(r)} = \frac{\delta E_{e}[n(r)]}{\delta n(r)}-V_{e}(r)=0,
\end{equation}
%

The variation with respect to the single particle states is equal to the potential:
%
\begin{equation}
\frac{\Phi^{TB}}{\delta \psi(r)} = \left[-\frac{1}{2}\nabla^{2} + \tilde{V}(r)\right]\psi_{i}(r)-\epsilon_{i}\psi_{i}(r)=0,
\end{equation}
%

and finally the variation with respect to the potential gives the constraint on the density:

\begin{equation}
\frac{\Phi^{TB}}{\delta V(r)} = \sum_{i}\psi^{*}_{i}(r)\psi_{i}(r) - n(\r) = 0
\end{equation}

The function $\Phi$ has been shown to be minimal with respect to $n(r)$ 
and $\psi_i$ and maximal with respect to $V_{e}(r)$ \cite{methfessel95}.
...
His development is far more thorought than that given here and makes
a direct connection with response functions. 

There is one unfortunate piece of nomenclature. The natural
contraction of the Harris-Foulkes functional would be HF.
This could lead to confusion with Hartree-Fock. So in the subsequent
discussion the Harris-Foulkes functional will contract to TB, short for,
the Tight-Binding functional.

The fact that $\Phi^{TB}[n]$ is stationary but not a strict minimum is perhaps not
surprising. Wishing it were otherwise would be greedy. 
The microscopic state of a real material is constantly evolving, 
the boundary conditions are also constantly varying, at some point the 
electronic density could be transformed and all the rules of the game
would change. A saddle point allows for these possibilities. The saddle
point acknowledges things can change, and {\it variatio delectat}
\cite{Latin. tr. Change is delicious}.

The best that can be expected is to use as much physical intuition and
observation as possible to arrive at a description of the electronic states
that is stable. From this stable configuration which gives us the `best'
description of the ground state we can introduce the theoretical probes
and perturbations to obtain the quantities of interest that were mentioned
in the introduction.

\section{Conclusion}
\noindent
This chapter introduced the Hohenberg-Kohn theorem. This theorem which 
states the ground-state energy of an interacting electronic system is a function of the 
ground-state charge density. From that theorem the rest follows.
In practice the Kohn-Sham scheme provides a prescription for obtaining 
a set of wavefunctions, and eigenvalues, thus describing
the ground-state density of a system. The various approximations to the exchange correlation 
functional commonly used in applications of Kohn-Sham DFT like the LDA, LDA+U,
GGA, and hybrid functionals are our work horses. For a given configuration of ions
it is commonplace to send a calculation off to a computing machine and within a few
...
and correlation in real systems. In particular we discussed an
...
amenable to local interactions was begun in the section on the Harris-Foulkes functional.
These idea will be developed further in Chapter~\ref{chap:manybodyrec}, but first 
we will look at a popular way of preparing localized wave packets in solids 
